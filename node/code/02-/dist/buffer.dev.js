...
var b1 = Buffer.alloc(10); // allocUnsafe:创建制定大小的buffer（不安全）

var b2 = Buffer.allocUnsafe(10); // console.log(b1);
// console.log(b2);
// from：接受数据，创建buffer
...
var b8 = Buffer.from(b1); // console.log(b8);

b1[0] = 2; // console.log(b8);
// buffer 实例方法

var buf = Buffer.alloc(6); // fill:使用数据填充buffer
// buf.fill('123',1,3) 
...
buf = Buffer.from('Jony'); // 1、从buffer开始到buffer结束存入数据
// 1、数据编码类型  
// 2、从buffer哪一个下标开始截取
// 3、结束位置
// slice截取buffer
...
console.log("buffer->", buf.toString()); // 静态方法
